...
function ListaFiltrosEventosPeriodicos({ filtros, areas = [] }) {
  ...
  const renderFiltros = () => {
    ...
    if (
      filtrosAtivos.modoCombinacao !== undefined &&
      filtrosAtivos.modoCombinacao !== null &&
      filtrosAtivos.modoCombinacao !== ''
    ) {
      items.push(
    ...
    if (
      Array.isArray(filtrosAtivos.adequacaoDefesas) &&
      filtrosAtivos.adequacaoDefesas.length > 0
    ) {
      const adequacoes = [];
