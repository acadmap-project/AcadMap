...
function FiltroEventosPeriodicos({ onResultados }) {
    ...
    const onSubmit = async (data) => {
        ...
        // Normaliza os dados do filtro
        const normalizedData = normalizeToNull(data);
        const params = new URLSearchParams();

        if (normalizedData.nome) params.append("nome", normalizedData.nome);

        try {
            const [eventos, periodicos] = await Promise.all([
                fetch(`http://localhost:8080/api/eventos/listar?${params.toString()}`),
                fetch(`http://localhost:8080/api/periodicos/listar?${params.toString()}`)
            ]);

            if (!eventos.ok || !periodicos.ok) {
                const eventosText = await eventos.text();
                const periodicosText = await periodicos.text();
                throw new Error(
                    `Erro ao buscar dados:\nEventos: ${eventos.status} - ${eventosText}\nPeriódicos: ${periodicos.status} - ${periodicosText}`
                );
            }

            const eventosData = await eventos.json();
            const periodicosData = await periodicos.json();

            setSuccess("Busca realizada com sucesso!");

            if (onResultados) {
                onResultados({
                    eventos: Array.isArray(eventosData) ? eventosData : [],
                    periodicos: Array.isArray(periodicosData) ? periodicosData : []
                });
            }
        } catch (err) {
    ...
    return (
        <form
            className="w-full max-w-3xl mx-auto grid grid-cols-1 gap-6 mt-8 text-left"
