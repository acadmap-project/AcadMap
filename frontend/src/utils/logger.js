...
class LoggerClass {
  ...
  async _processRetryQueue() {
    ...
    console.warn(
      `[LOGGER] Tentando reenviar ${this._pendingLogs.length} logs pendentes...`
    );
    ...
    for (const logItem of logsToRetry) {
      ...
      try {
        ...
        console.log(
          `[${logItem.logType}] Reenvio ${logItem.attempts}/3 ${requireAuth ? 'COM' : 'SEM'} autenticação`
        );

        await post(logItem.endpoint, logItem.payload, {}, requireAuth);
        console.warn(
          `[${logItem.logType}] Log reenviado com sucesso após ${logItem.attempts} tentativa(s)`
        );
      } catch (error) {
        ...
        if (logItem.attempts < 3) {
          ...
          console.warn(
            `[${logItem.logType}] Falha no reenvio (tentativa ${logItem.attempts}/3):`,
            error
          );
        } else {
          console.warn(
            `[${logItem.logType}] Log descartado após 3 tentativas falharam:`,
            error
          );
        }
      }
  ...
  async _sendLog(endpoint, payload, logType) {
    try {
      const requireAuth = this._isUserAuthenticated();
      console.log(
        `[${logType}] Enviando log ${requireAuth ? 'COM' : 'SEM'} autenticação`
      );
